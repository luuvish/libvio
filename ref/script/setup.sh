...
if [ ! -d $LIBVPX_SRC ]; then
  git clone http://git.chromium.org/webm/libvpx.git $LIBVPX_SRC
  [ ! -d $LIBVPX_SRC ] && exit 1
...
if [ ! -d $OPENHEVC_SRC ]; then
  git clone https://github.com/OpenHEVC/openHEVC $OPENHEVC_SRC
...
if [ ! -d $OPENH264_SRC ]; then
  git clone https://github.com/cisco/openh264 $OPENH264_SRC
...
if [ ! -d $H265HM_SRC ]; then
  svn checkout https://hevc.hhi.fraunhofer.de/svn/svn_HEVCSoftware/tags/HM-14.0+RExt-7.2/ $H265HM_SRC
  [ ! -d $H265HM_SRC ] && exit 1
  (svn update r3977)
fi
...
if [ ! -f $H264JM_ZIP ]; then
  curl http://iphome.hhi.de/suehring/tml/download/jm$H264JM_VER.zip > $H264JM_ZIP
  [ ! -f $H264JM_ZIP ] && exit 1
fi
...
# SMPTE_VC1_SRC="$TARGET_DIR/VC-1-SourceCode-$SMPTE_VC1_VER"
# SMPTE_VC1_BIN="$TARGET_DIR/VC-1-SourceCode-$SMPTE_VC1_VER"
# 
# if [ ! -d $SMPTE_VC1_SRC ]; then
#   (unzip $SMPTE_VC1_ZIP; mv VC-1-SourceCode-$SMPTE_VC1_VER $SMPTE_VC1_SRC)
...
#   cd $SPARROW_BIN;
#   patch -p0 -u < $SCRIPT_DIR/smpte-vc1.diff;
#   make;
#   cd $SCRIPT_DIR;
# )
...
(
  ...
  patch -p0 < $SCRIPT_DIR/coda960.diff;
  make -f $SCRIPT_DIR/coda960.mk;
  cd $SCRIPT_DIR;
)
