...
void data_partition_t::slice_header(slice_t& slice)
{
    ...
    p_Vid->type = shr.slice_type = this->ue("SH: slice_type") % 5;
    shr.pic_parameter_set_id = this->ue("SH: pic_parameter_set_id");

    UseParameterSet(&slice);
    slice.active_sps = p_Vid->active_sps;
    slice.active_pps = p_Vid->active_pps;
...
#if (MVC_EXTENSION_ENABLE)
    else if ( slice.svc_extension_flag == 0 && slice.NaluHeaderMVCExt.non_idr_flag == 0 )
        shr.idr_pic_id = this->ue("SH: idr_pic_id");
#endif

    assert(shr.idr_pic_id <= 65535);
    ...
    assert(shr.redundant_pic_cnt <= 127);
    ...
    if (!shr.field_pic_flag)
        assert(shr.num_ref_idx_l0_active_minus1 <= 15);
    else
        assert(shr.num_ref_idx_l0_active_minus1 <= 31);
    if (!shr.field_pic_flag)
        assert(shr.num_ref_idx_l1_active_minus1 <= 15);
    else
        assert(shr.num_ref_idx_l1_active_minus1 <= 31);
    ...
    if (slice.nal_ref_idc != 0)
        this->dec_ref_pic_marking(p_Vid, slice);
...
void data_partition_t::ref_pic_list_modification(slice_t& slice)
{
    ...
    if (shr.slice_type != I_slice && shr.slice_type != SI_slice) {
        ...
        if (shr.ref_pic_list_modification_flag_l0) {
            int i = 0;
            do {
                shr.modification_of_pic_nums_idc[0][i] = this->ue("SH: modification_of_pic_nums_idc_l0");
                if (shr.modification_of_pic_nums_idc[0][i] == 0 ||
                    shr.modification_of_pic_nums_idc[0][i] == 1)
                    shr.abs_diff_pic_num_minus1[0][i] = this->ue("SH: abs_diff_pic_num_minus1_l0");
                else if (shr.modification_of_pic_nums_idc[0][i] == 2)
                    shr.long_term_pic_num[0][i] = this->ue("SH: long_term_pic_idx_l0");
    ...
    if (shr.slice_type == B_slice) {
        ...
        if (shr.ref_pic_list_modification_flag_l1) {
            int i = 0;
            do {
                shr.modification_of_pic_nums_idc[1][i] = this->ue("SH: modification_of_pic_nums_idc_l1");
                if (shr.modification_of_pic_nums_idc[1][i] == 0 ||
                    shr.modification_of_pic_nums_idc[1][i] == 1)
                    shr.abs_diff_pic_num_minus1[1][i] = this->ue("SH: abs_diff_pic_num_minus1_l1");
                else if (shr.modification_of_pic_nums_idc[1][i] == 2)
                    shr.long_term_pic_num[1][i] = this->ue("SH: long_term_pic_idx_l1");
        ...
        if (shr.ref_pic_list_modification_flag_l0) {
            ...
            do {
                ...
                if (shr.modification_of_pic_nums_idc[0][i] == 0 ||
                    ...
                    shr.abs_diff_view_idx_minus1[0][i] = this->ue("SH: abs_diff_view_idx_minus1_l0");
            } while (shr.modification_of_pic_nums_idc[0][i++] != 3);
        }
    }
        ...
        if (shr.ref_pic_list_modification_flag_l1) {
            ...
            do {
                ...
                if (shr.modification_of_pic_nums_idc[1][i] == 0 ||
                    ...
                    shr.abs_diff_view_idx_minus1[1][i] = this->ue("SH: abs_diff_view_idx_minus1_l1");
            } while (shr.modification_of_pic_nums_idc[1][i++] != 3);
        }
    }
}
...
void data_partition_t::pred_weight_table(slice_t& slice)
{
    ...
    for (int i = 0; i <= shr.num_ref_idx_l0_active_minus1; ++i) {
        shr.luma_weight_l0_flag[i] = this->u(1, "SH: luma_weight_flag_l0");
        shr.luma_weight_l0     [i] = 1 << shr.luma_log2_weight_denom;
        shr.luma_offset_l0     [i] = 0;
        if (shr.luma_weight_l0_flag[i]) {
            shr.luma_weight_l0[i] = this->se("SH: luma_weight_l0");
            shr.luma_offset_l0[i] = this->se("SH: luma_offset_l0");
        }
        ...
        if (sps.ChromaArrayType != 0) {
            shr.chroma_weight_l0_flag[i] = this->u(1, "SH: chroma_weight_flag_l0");
            for (int comp = 0; comp < 2; ++comp) {
                ...
                if (shr.chroma_weight_l0_flag[i]) {
                    shr.chroma_weight_l0[i][comp] = this->se("SH: chroma_weight_l0");
                    shr.chroma_offset_l0[i][comp] = this->se("SH: chroma_offset_l0");
                }
    ...
    for (int i = 0; i <= shr.num_ref_idx_l1_active_minus1; ++i) {
        shr.luma_weight_l1_flag[i] = this->u(1, "SH: luma_weight_flag_l1");
        shr.luma_weight_l1     [i] = 1 << shr.luma_log2_weight_denom;
        shr.luma_offset_l1     [i] = 0;
        if (shr.luma_weight_l1_flag[i]) {
            shr.luma_weight_l1[i] = this->se("SH: luma_weight_l1");
            shr.luma_offset_l1[i] = this->se("SH: luma_offset_l1");
        }
        ...
        if (sps.ChromaArrayType != 0) {
            shr.chroma_weight_l1_flag[i] = this->u(1, "SH: chroma_weight_flag_l1");
            for (int comp = 0; comp < 2; ++comp) {
                shr.chroma_weight_l1[i][comp] = 1 << shr.chroma_log2_weight_denom;
                shr.chroma_offset_l1[i][comp] = 0;
                if (shr.chroma_weight_l1_flag[i]) {
                    shr.chroma_weight_l1[i][comp] = this->se("SH: chroma_weight_l1");
                    shr.chroma_offset_l1[i][comp] = this->se("SH: chroma_offset_l1");
                }
...
void data_partition_t::dec_ref_pic_marking(VideoParameters* p_Vid, slice_t& slice)
{
    sps_t& sps = *slice.active_sps;
    shr_t& shr = slice.header;
    int val;
    ...
    if (slice.idr_flag
        ...
        if (shr.adaptive_ref_pic_marking_mode_flag) {
            do {
                ...
                val = tmp_drpm->memory_management_control_operation = this->ue("SH: memory_management_control_operation");

                if (val == 1 || val == 3)
                    tmp_drpm->difference_of_pic_nums_minus1 = this->ue("SH: difference_of_pic_nums_minus1");
                if (val == 2)
                    tmp_drpm->long_term_pic_num = this->ue("SH: long_term_pic_num");

                if (val == 3 || val == 6)
                    tmp_drpm->long_term_frame_idx = this->ue("SH: long_term_frame_idx");
                if (val == 4)
                    tmp_drpm->max_long_term_frame_idx_plus1 = this->ue("SH: max_long_term_pic_idx_plus1");
